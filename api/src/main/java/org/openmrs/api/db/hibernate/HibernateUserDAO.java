...
public class HibernateUserDAO implements UserDAO {
	...
	public User saveUser(User user, String password) {
		...
		if (isNewUser && password != null) {
			/*		enMRS, we are using generation strategy as native which will convert to IDENTITY 
				r MySQL and SEQUENCE for PostgreSQL. When using IDENTITY strategy, hibernate directly 
				sues insert statements where as with  SEQUENCE strategy hibernate only increments 
				quences and issues insert on session flush ( batching is possible) . 
				stgreSQL behaves differently than MySQL because it makes use of SEQUENCE strategy. 
			*/
